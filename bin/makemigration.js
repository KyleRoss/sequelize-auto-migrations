...
try {
    previousState = JSON.parse(fs.readFileSync(path.join(stateDir, '_current.json')));
} catch(e) { /**/ }
...
if(fs.existsSync(path.join(stateDir, '_current.json')))
    fs.writeFileSync(path.join(stateDir, '_current_bak.json'),
        fs.readFileSync(path.join(stateDir, '_current.json'))
    );


// save current state
currentState.revision = previousState.revision + 1;
fs.writeFileSync(path.join(stateDir, '_current.json'), JSON.stringify(currentState, null, 4));
