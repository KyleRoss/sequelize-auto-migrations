...
const sortActions = function(actions)
{
    ...
    actions.sort((a, b) => {
        if(['createTable', 'dropTable'].includes(a.actionType) === false) return 0;
        if(a.tableName.match(/^users?$/i)) return -1;
        return 0;
    });
            ...
            return new Promise(function(resolve, reject) {
                function next() {
                    if(index < commands.length) {
                        ...
                        index += 1;
                        let method = queryInterface;
                        if(command.fn === 'query') method = method.sequelize.query;
                        else method = method[command.fn];
